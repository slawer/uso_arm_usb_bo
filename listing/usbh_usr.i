...
extern volatile uint8_t AudioPlayStart ;
uint8_t joystick_use = 0x00;
uint8_t lcdLineNo = 0x00;
extern volatile uint8_t RepeatState ;
extern volatile uint8_t LED_Toggle;
static uint8_t USBH_USR_ApplicationState = ((uint8_t)0x00);

extern volatile uint16_t Time_Rec_Base;
...
void USBH_USR_DeviceAttached(void)
{
	...
	sost_flesh=1;
	((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x1000))->BSRRL = ((uint16_t)0x0010);  
	((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x1000))->BSRRH = ((uint16_t)0x0020);  
...
void USBH_USR_DeviceDisconnected (void)
{
		...
		sost_flesh=0;	
		((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x1000))->BSRRH = ((uint16_t)0x0010);  
		((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x1000))->BSRRL = ((uint16_t)0x0020);  
  ...
  TIM_ITConfig(((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0800)), ((uint16_t)0x0002) , DISABLE);
...
int USBH_USR_MSC_Application(void)
{
  ...
  switch (USBH_USR_ApplicationState)
  {
    case ((uint8_t)0x00):
      ...
      USBH_USR_ApplicationState = ((uint8_t)0x01);
      break;
    ...
    case ((uint8_t)0x01):
      ...
      USBH_USR_ApplicationState = ((uint8_t)0x00);
      break;
...
if (buffering)
	...
	Buf_zap[6]=(uint8_t)(DT_zap.Seconds/10)+(uint8_t)0x30;
      ...
      {			
				...
				Buf_zap[sm+7*kol_zap]=0x3B; 
			 dec_to_chr(Buf_adc_zap1[kol_zap],(uint8_t*) &Buf_zap[sm+7*kol_zap+1]);			
				kol_zap++;			
				Buf_zap[sm+7*kol_zap-1]=0x0D;	
			}
		else
			while (kol_zap<600)
      {			
      ...
      {			
				...
				Buf_zap[sm+7*kol_zap]=0x3B; 
			 dec_to_chr(Buf_adc_zap2[kol_zap],(uint8_t*) &Buf_zap[sm+7*kol_zap+1]);			
				kol_zap++;			
				Buf_zap[sm+7*kol_zap-1]=0x0D;	
			}		  	
			}		
				...
				if (HCD_IsDeviceConnected(&USB_OTG_Core) == 1)
				{
					...
					{
					if (f_open(&file,  (const XCHAR *)&file_name, 0x02)== FR_OK)	
				
						{
							STM_EVAL_LEDOn(LED4);
							f_lseek(&file, (DWORD)(file.fsize));
							f_write (&file, (uint8_t*)Buf_zap, sm+7*kol_zap, (void *)&bytesWritten); 				
							f_close (&file);
						...
						{
							if (f_open(&file, (const XCHAR *)&file_name, 0x08)== FR_OK)

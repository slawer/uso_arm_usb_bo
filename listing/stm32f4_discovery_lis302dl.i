...
#line 25 "Utilities\\STM32F4-Discovery\\stm32f4_discovery_lis302dl.c"
...
volatile uint32_t  LIS302DLTimeout = ((uint32_t)0x1000);   
...
void LIS302DL_Init(LIS302DL_InitTypeDef *LIS302DL_InitStruct)
{
  ...
  ctrl = (uint8_t) (LIS302DL_InitStruct->Output_DataRate | LIS302DL_InitStruct->Power_Mode |                     LIS302DL_InitStruct->Full_Scale | LIS302DL_InitStruct->Self_Test |                     LIS302DL_InitStruct->Axes_Enable);


  
   
  LIS302DL_Write(&ctrl, 0x20, 1);
}
...
void LIS302DL_FilterConfig(LIS302DL_FilterConfigTypeDef *LIS302DL_FilterConfigStruct)
{
  ...
  LIS302DL_Read(&ctrl, 0x21, 1);
  
   
  ctrl &= (uint8_t)~(((uint8_t)0x20) |                      ((uint8_t)0x03) |                      ((uint8_t)0x0C));


                        
  ctrl |= (uint8_t)(LIS302DL_FilterConfigStruct->HighPassFilter_Data_Selection |                     LIS302DL_FilterConfigStruct->HighPassFilter_CutOff_Frequency |                     LIS302DL_FilterConfigStruct->HighPassFilter_Interrupt);


  
   
  LIS302DL_Write(&ctrl, 0x21, 1);
}
...
void LIS302DL_InterruptConfig(LIS302DL_InterruptConfigTypeDef *LIS302DL_IntConfigStruct)
{
  ...
  LIS302DL_Read(&ctrl, 0x38, 1);
  
                      
  ctrl = (uint8_t)(LIS302DL_IntConfigStruct->Latch_Request|                    LIS302DL_IntConfigStruct->SingleClick_Axes |                    LIS302DL_IntConfigStruct->DoubleClick_Axes);


  
   
  LIS302DL_Write(&ctrl, 0x38, 1);
}
...
void LIS302DL_LowpowerCmd(uint8_t LowPowerMode)
{
  ...
  LIS302DL_Read(&tmpreg, 0x20, 1);
  
   
  tmpreg &= (uint8_t)~((uint8_t)0x40);
  tmpreg |= LowPowerMode;
  
   
  LIS302DL_Write(&tmpreg, 0x20, 1);
}
...
void LIS302DL_RebootCmd(void)
{
  ...
  LIS302DL_Read(&tmpreg, 0x21, 1);
  
   
  tmpreg |= ((uint8_t)0x40);
  
   
  LIS302DL_Write(&tmpreg, 0x21, 1);
}
...
void LIS302DL_Write(uint8_t* pBuffer, uint8_t WriteAddr, uint16_t NumByteToWrite)
{
  ...
  GPIO_ResetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x1000)), ((uint16_t)0x0008));
  ...
  GPIO_SetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x1000)), ((uint16_t)0x0008));
}
...
{  
  ...
  while(NumByteToRead > 0x00)
  {
    ...
    *pBuffer = LIS302DL_SendByte(((uint8_t)0x00));
    NumByteToRead--;
    pBuffer++;
  }
...
void LIS302DL_ReadACC(int32_t* out)
{
  ...
  switch(crtl & 0x20) 
    ...
    case 0x00:
      for(i=0; i<0x03; i++)
      {
        *out =(int32_t)(18 *  (int8_t)buffer[2*i]);
        out++;
      }
      break;
...
static void LIS302DL_LowLevel_Init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(((uint32_t)0x00000001) | ((uint32_t)0x00000001) | ((uint32_t)0x00000001), ENABLE);
...
static uint8_t LIS302DL_SendByte(uint8_t byte)
{
  ...
  LIS302DLTimeout = ((uint32_t)0x1000);
  while (SPI_I2S_GetFlagStatus(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x3000)), ((uint16_t)0x0002)) == RESET)
  {
    if((LIS302DLTimeout--) == 0) return 0; 
  }
  
   
  SPI_I2S_SendData(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x3000)), byte);
  ...
  return (uint8_t)SPI_I2S_ReceiveData(((SPI_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x3000)));
}

#line 484 "Utilities\\STM32F4-Discovery\\stm32f4_discovery_lis302dl.c"

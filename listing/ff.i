...
#line 80 "Utilities\\Third_Party\\fat_fs\\ff.c"
#line 1 "Utilities\\Third_Party\\fat_fs\\diskio.h"
...
#line 11 "Utilities\\Third_Party\\fat_fs\\diskio.h"
...
#line 12 "Utilities\\Third_Party\\fat_fs\\diskio.h"
...
#line 57 "Utilities\\Third_Party\\fat_fs\\diskio.h"
...
#line 81 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
#line 103 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
#line 120 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
#line 158 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
#line 236 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
)
{
	...
	case 1 :
		bc = clst; bc += bc / 2;
		if (move_window(fs, fsect + (bc / ((fs)->s_size)))) break;
		wc = fs->win[bc & (((fs)->s_size) - 1)]; bc++;
		if (move_window(fs, fsect + (bc / ((fs)->s_size)))) break;
		wc |= (WORD)fs->win[bc & (((fs)->s_size) - 1)] << 8;
		return (clst & 1) ? (wc >> 4) : (wc & 0xFFF);

	case 2 :
		if (move_window(fs, fsect + (clst / (((fs)->s_size) / 2)))) break;
		return (WORD)(((WORD)*(BYTE*)((&fs->win[((WORD)clst * 2) & (((fs)->s_size) - 1)])+1)<<8)|(WORD)*(BYTE*)(&fs->win[((WORD)clst * 2) & (((fs)->s_size) - 1)]));

	case 3 :
		if (move_window(fs, fsect + (clst / (((fs)->s_size) / 4)))) break;
		return (DWORD)(((DWORD)*(BYTE*)((&fs->win[((WORD)clst * 4) & (((fs)->s_size) - 1)])+3)<<24)|((DWORD)*(BYTE*)((&fs->win[((WORD)clst * 4) & (((fs)->s_size) - 1)])+2)<<16)|((WORD)*(BYTE*)((&fs->win[((WORD)clst * 4) & (((fs)->s_size) - 1)])+1)<<8)| *(BYTE*)(&fs->win[((WORD)clst * 4) & (((fs)->s_size) - 1)])) & 0x0FFFFFFF;
	}
...
)
{
	...
	if (clst < 2 || clst >= fs->max_clust) {	 
		...
		case 1 :
			bc = clst; bc += bc / 2;
			res = move_window(fs, fsect + (bc / ((fs)->s_size)));
			if (res != FR_OK) break;
			p = &fs->win[bc & (((fs)->s_size) - 1)];
		...
		case 2 :
			res = move_window(fs, fsect + (clst / (((fs)->s_size) / 2)));
			if (res != FR_OK) break;
			*(BYTE*)(&fs->win[((WORD)clst * 2) & (((fs)->s_size) - 1)])=(BYTE)((WORD)val); *(BYTE*)((&fs->win[((WORD)clst * 2) & (((fs)->s_size) - 1)])+1)=(BYTE)((WORD)((WORD)val)>>8);
			break;

		case 3 :
			res = move_window(fs, fsect + (clst / (((fs)->s_size) / 4)));
			if (res != FR_OK) break;
			*(BYTE*)(&fs->win[((WORD)clst * 4) & (((fs)->s_size) - 1)])=(BYTE)(val); *(BYTE*)((&fs->win[((WORD)clst * 4) & (((fs)->s_size) - 1)])+1)=(BYTE)((WORD)(val)>>8); *(BYTE*)((&fs->win[((WORD)clst * 4) & (((fs)->s_size) - 1)])+2)=(BYTE)((DWORD)(val)>>16); *(BYTE*)((&fs->win[((WORD)clst * 4) & (((fs)->s_size) - 1)])+3)=(BYTE)((DWORD)(val)>>24);
			break;
...
)
{
	...
	if (clst == 0) {	 
		...
		if (idx >= dj->fs->n_rootdir)		 
			return FR_INT_ERR;
		dj->sect = dj->fs->dirbase + idx / (((dj->fs)->s_size) / 32);	 
	}
	else {				 
		ic = ((dj->fs)->s_size) / 32 * dj->fs->csize;	 
		while (idx >= ic) {	 
		...
		dj->clust = clst;
		dj->sect = clust2sect(dj->fs, clst) + idx / (((dj->fs)->s_size) / 32);	 
	}

	dj->dir = dj->fs->win + (idx % (((dj->fs)->s_size) / 32)) * 32;	 
...
)
{
	...
	if (!(i % (((dj->fs)->s_size) / 32))) {	 
		...
		if (dj->clust == 0) {	 
			...
			if (((i / (((dj->fs)->s_size) / 32)) & (dj->fs->csize - 1)) == 0) {	 
				...
				if (clst >= dj->fs->max_clust) {				 
					...
					mem_set(dj->fs->win, 0, ((dj->fs)->s_size));			 
	...
	dj->index = i;
	dj->dir = dj->fs->win + (i % (((dj->fs)->s_size) / 32)) * 32;
...
#line 758 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
#line 802 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
#line 822 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
#line 875 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
)
{
	...
	while (dj->sect) {
		...
		c = dir[0];
		if (c == 0) { res = FR_NO_FILE; break; }	 
#line 929 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
)
{
	FRESULT res;
	BYTE c, *dir;
#line 1018 "Utilities\\Third_Party\\fat_fs\\ff.c"
	res = dir_seek(dj, 0);
	if (res == FR_OK) {
		do {	 
			res = move_window(dj->fs, dj->sect);
...
{
	FRESULT res;
#line 1076 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
#line 1229 "Utilities\\Third_Party\\fat_fs\\ff.c"
	...
	si = i = b = 0; ni = 8;
	p = *path;
#line 1251 "Utilities\\Third_Party\\fat_fs\\ff.c"
	for (;;) {
		c = p[si++];
		if (c <= ' ' || c == '/' || c == '\\') break;	 
		if (c == '.' || i >= ni) {
		...
		if (c >= 0x80) {				 
			...
			c = cvt[c - 0x80];			 
#line 1268 "Utilities\\Third_Party\\fat_fs\\ff.c"
		}
		if (0) {				 
			d = p[si++];				 
			if (!0 || i >= ni - 1)	 
...
#line 1371 "Utilities\\Third_Party\\fat_fs\\ff.c"
}
...
)
{
	...
	while (!0 && *path == ' ') path++;	 
#line 1398 "Utilities\\Third_Party\\fat_fs\\ff.c"
	if (*path == '/' || *path == '\\')	 
		path++;
	dj->sclust = 0;						 
...
)
{
	...
	*rfs = fs = FatFs[vol];			 
	...
	if (disk_ioctl(fs->drive, 2, &((fs)->s_size)) != RES_OK || ((fs)->s_size) > 1024)
	...
	if (fmt || (WORD)(((WORD)*(BYTE*)((fs->win+11)+1)<<8)|(WORD)*(BYTE*)(fs->win+11)) != ((fs)->s_size))	 
		return FR_NO_FILESYSTEM;
	...
	fs->max_clust = mclst = (tsect						 
		- (WORD)(((WORD)*(BYTE*)((fs->win+14)+1)<<8)|(WORD)*(BYTE*)(fs->win+14)) - fsize - fs->n_rootdir / (((fs)->s_size)/32)
		) / fs->csize + 2;
	...
	fs->database = fs->fatbase + fsize + fs->n_rootdir / (((fs)->s_size)/32);	 
...
)
{
	...
	*br = 0;	 
	...
	for ( ;  btr;									 
		...
		if ((fp->fptr % ((fp->fs)->s_size)) == 0) {			 
			...
			if (cc) {								 
				...
				if ((fp->flag & 0x40) && fp->dsect - sect < cc)	 
					mem_cpy(rbuff + ((fp->dsect - sect) * ((fp->fs)->s_size)), fp->buf, ((fp->fs)->s_size));


				fp->csect += (BYTE)cc;				 
				rcnt = ((fp->fs)->s_size) * cc;				 
				continue;
			}
		...
		rcnt = ((fp->fs)->s_size) - (fp->fptr % ((fp->fs)->s_size));	 
		...
		mem_cpy(rbuff, &fp->buf[fp->fptr % ((fp->fs)->s_size)], rcnt);	 
		...
		if ((fp->fptr % ((fp->fs)->s_size)) == 0) {			 
			...
			if (cc) {								 
				...
				if (disk_write(fp->fs->drive, wbuff, sect, (BYTE)cc) != RES_OK)
					{ fp->flag |= 0x80; return FR_DISK_ERR; };
#line 1923 "Utilities\\Third_Party\\fat_fs\\ff.c"
				if (fp->dsect - sect < cc) {		 
					mem_cpy(fp->buf, wbuff + ((fp->dsect - sect) * ((fp->fs)->s_size)), ((fp->fs)->s_size));
					fp->flag &= ~0x40;
				}

				fp->csect += (BYTE)cc;				 
				wcnt = ((fp->fs)->s_size) * cc;				 
				continue;
			}
#line 1938 "Utilities\\Third_Party\\fat_fs\\ff.c"
			if (fp->dsect != sect) {				 
				if (fp->fptr < fp->fsize &&
		...
		wcnt = ((fp->fs)->s_size) - (fp->fptr % ((fp->fs)->s_size));	 
		if (wcnt > btw) wcnt = btw;
#line 1955 "Utilities\\Third_Party\\fat_fs\\ff.c"
		mem_cpy(&fp->buf[fp->fptr % ((fp->fs)->s_size)], wbuff, wcnt);	 
		fp->flag |= 0x40;
...
#line 2094 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
)
{
	...
	if (ofs > 0) {
		bcs = (DWORD)fp->fs->csize * ((fp->fs)->s_size);	 
		if (ifptr > 0 &&
		...
		if (clst != 0) {
			...
			fp->fptr += ofs;
			fp->csect = (BYTE)(ofs / ((fp->fs)->s_size));	 
			if (ofs % ((fp->fs)->s_size)) {
	...
	if (fp->fptr % ((fp->fs)->s_size) && nsect != fp->dsect) {
...
#line 2766 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
)
{
	...
	if (disk_ioctl(drv, 2, &((fs)->s_size)) != RES_OK
		|| ((fs)->s_size) > 1024)
	...
	if (!allocsize) {					 
		d = n_part;
		for (as = ((fs)->s_size); as > 512U; as >>= 1) d >>= 1;
		for (n = 0; d < sstbl[n]; n++) ;
		allocsize = cstbl[n];
	}
	if (allocsize < ((fs)->s_size)) allocsize = ((fs)->s_size);
	...
	switch (fmt) {
	case 1:
		n_fat = ((n_clst * 3 + 1) / 2 + 3 + ((fs)->s_size) - 1) / ((fs)->s_size);
		n_rsv = 1 + partition;
		n_dir = 512 * 32 / ((fs)->s_size);
		break;
	case 2:
		n_fat = ((n_clst * 2) + 4 + ((fs)->s_size) - 1) / ((fs)->s_size);
		n_rsv = 1 + partition;
		n_dir = 512 * 32 / ((fs)->s_size);
		break;
	default:
		n_fat = ((n_clst * 4) + 8 + ((fs)->s_size) - 1) / ((fs)->s_size);
		n_rsv = 33 - partition;
		n_dir = 0;
	}
	b_fat = b_part + n_rsv;			 
	...
	*(BYTE*)(tbl+11)=(BYTE)(((fs)->s_size)); *(BYTE*)((tbl+11)+1)=(BYTE)((WORD)(((fs)->s_size))>>8);		 
	tbl[13] = (BYTE)allocsize;		 
	*(BYTE*)(tbl+14)=(BYTE)(n_rsv); *(BYTE*)((tbl+14)+1)=(BYTE)((WORD)(n_rsv)>>8);			 
	tbl[16] = 1;					 
	*(BYTE*)(tbl+17)=(BYTE)(((fs)->s_size) / 32 * n_dir); *(BYTE*)((tbl+17)+1)=(BYTE)((WORD)(((fs)->s_size) / 32 * n_dir)>>8);  
	if (n_part < 0x10000) {						 
	...
	*(BYTE*)(tbl+510)=(BYTE)(0xAA55); *(BYTE*)((tbl+510)+1)=(BYTE)((WORD)(0xAA55)>>8);				 
	if (((fs)->s_size) > 512U) {
		*(BYTE*)(tbl+((fs)->s_size)-2)=(BYTE)(0xAA55); *(BYTE*)((tbl+((fs)->s_size)-2)+1)=(BYTE)((WORD)(0xAA55)>>8);
	}
...
#line 3023 "Utilities\\Third_Party\\fat_fs\\ff.c"
...
)
{
	...
	for (cc = res = 0; cc != -1; res += cc) {
		...
		if (c == 's') {				 
			cc = f_puts(__va_arg(arp, char*), fil);
			continue;
		}
		if (c == 'c') {				 
			cc = f_putc(__va_arg(arp, int), fil);
			if (cc != -1) cc = 1;
			continue;
		}
		r = 0;
		...
		if (r == 0) break;			 
		if (f & 2) {				 
			val = (ULONG)__va_arg(arp, long);
		} else {
			val = (c == 'd') ? (ULONG)(long)__va_arg(arp, int) : (ULONG)__va_arg(arp, unsigned int);
		}

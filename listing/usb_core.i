...
#line 24 "Libraries\\STM32_USB_OTG_Driver\\usb_core.c"
...
#line 25 "Libraries\\STM32_USB_OTG_Driver\\usb_core.c"
...
#line 264 "Libraries\\STM32_USB_OTG_Driver\\usb_core.c"
...
USB_OTG_STS USB_OTG_CoreInit(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (pdev->cfg.phy_itface == 1)
  {
    ...
    usbcfg.b.physel            = 0;  
#line 358 "Libraries\\STM32_USB_OTG_Driver\\usb_core.c"
...
#line 454 "Libraries\\STM32_USB_OTG_Driver\\usb_core.c"
  return status;
}
...
USB_OTG_STS USB_OTG_CoreInitHost(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  hcfg.b.fslssupp = 0;
  (*(volatile uint32_t *)&pdev->regs . HREGS->HCFG = hcfg . d32);
  ...
  if(pdev->cfg.coreID == USB_OTG_FS_CORE_ID)
  {
    ...
    nptxfifosize.b.depth = 96;  
    (*(volatile uint32_t *)&pdev->regs . GREGS->DIEPTXF0_HNPTXFSIZ = nptxfifosize . d32);
    
    ptxfifosize.b.startaddr = 128 + 96;
    ptxfifosize.b.depth     = 96;
    (*(volatile uint32_t *)&pdev->regs . GREGS->HPTXFSIZ = ptxfifosize . d32);      
  }
#line 709 "Libraries\\STM32_USB_OTG_Driver\\usb_core.c"
  ...
  for (i = 0; i < pdev->cfg.host_channels; i++)
  {
    (*(volatile uint32_t *)&pdev->regs . HC_REGS[i]->HCINT = 0xFFFFFFFF);
    (*(volatile uint32_t *)&pdev->regs . HC_REGS[i]->HCGINTMSK = 0);
  }
...
uint8_t USB_OTG_IsEvenFrame (USB_OTG_CORE_HANDLE *pdev) 
{
  return !((*(volatile uint32_t *)&pdev->regs . HREGS->HFNUM) & 0x1);
}
...
void USB_OTG_DriveVbus (USB_OTG_CORE_HANDLE *pdev, uint8_t state)
{
  ...
  {
    hprt0.b.prtpwr = 1;
    (*(volatile uint32_t *)pdev->regs . HPRT0 = hprt0 . d32);
  }
...
uint32_t USB_OTG_ReadHostAllChannels_intr (USB_OTG_CORE_HANDLE *pdev)
{
  return ((*(volatile uint32_t *)&pdev->regs . HREGS->HAINT));
}
...
USB_OTG_STS USB_OTG_HC_Init(USB_OTG_CORE_HANDLE *pdev , uint8_t hc_num)
{
  ...
  (*(volatile uint32_t *)&pdev->regs . HC_REGS[hc_num]->HCINT = hcint . d32);
  ...
  (*(volatile uint32_t *)&pdev->regs . HC_REGS[hc_num]->HCGINTMSK = hcintmsk . d32);
  
  
   
  intr_enable = (1 << hc_num);
  (*(volatile uint32_t *)&pdev->regs . HREGS->HAINTMSK = ((((*(volatile uint32_t *)&pdev->regs . HREGS->HAINTMSK)) & ~0) | intr_enable ));
  
   
  gintmsk.b.hcintr = 1;
  (*(volatile uint32_t *)&pdev->regs . GREGS->GINTMSK = ((((*(volatile uint32_t *)&pdev->regs . GREGS->GINTMSK)) & ~0) | gintmsk . d32 ));
  ...
  hcchar.b.lspddev = (pdev->host.hc[hc_num].speed == 2);
  hcchar.b.eptype  = pdev->host.hc[hc_num].ep_type;
  hcchar.b.mps     = pdev->host.hc[hc_num].max_packet;
  if (pdev->host.hc[hc_num].ep_type == 3)
  {
    hcchar.b.oddfrm  = 1;
  }
  (*(volatile uint32_t *)&pdev->regs . HC_REGS[hc_num]->HCCHAR = hcchar . d32);
  return status;
}
...
USB_OTG_STS USB_OTG_HC_StartXfer(USB_OTG_CORE_HANDLE *pdev , uint8_t hc_num)
{
  ...
  (*(volatile uint32_t *)&pdev->regs . HC_REGS[hc_num]->HCTSIZ = hctsiz . d32);
  
  if (pdev->cfg.dma_enable == 1)
  {
    (*(volatile uint32_t *)&pdev->regs . HC_REGS[hc_num]->HCDMA = (unsigned int)pdev->host . hc[hc_num]. xfer_buff);
  }
  
  
  hcchar.d32 = (*(volatile uint32_t *)&pdev->regs . HC_REGS[hc_num]->HCCHAR);
...
USB_OTG_STS USB_OTG_HC_Halt(USB_OTG_CORE_HANDLE *pdev , uint8_t hc_num)
{
  ...
  if (hcchar.b.eptype == 0 || hcchar.b.eptype == 2)
  {
    nptxsts.d32 = (*(volatile uint32_t *)&pdev->regs . GREGS->HNPTXSTS);
...
void USB_OTG_StopHost(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  (*(volatile uint32_t *)&pdev->regs . HREGS->HAINTMSK = 0);
  (*(volatile uint32_t *)&pdev->regs . HREGS->HAINT = 0xFFFFFFFF);
  ...
  for (i = 0; i < pdev->cfg.host_channels; i++)
  {
    ...
    hcchar.b.chdis = 1;
    hcchar.b.epdir = 0;
    (*(volatile uint32_t *)&pdev->regs . HC_REGS[i]->HCCHAR = hcchar . d32);
  }
...
#line 2175 "Libraries\\STM32_USB_OTG_Driver\\usb_core.c"

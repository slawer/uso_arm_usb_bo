...
#line 24 "Libraries\\STM32_USB_HOST_Library\\usbh_hcs.c"
...
uint8_t USBH_Alloc_Channel  (USB_OTG_CORE_HANDLE *pdev, uint8_t ep_addr)
{
  ...
  if (hc_num != 0xFFFF)
  {
	pdev->host.channel[hc_num] = 0x8000 | ep_addr;
  }
  return hc_num;
}
...
static uint16_t USBH_GetFreeChannel (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  for (idx = 0 ; idx < 8 ; idx++)
  {
	if ((pdev->host.channel[idx] & 0x8000) == 0)
	{
	   return idx;
	} 
  }
